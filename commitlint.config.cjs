...
const scopesPackages = fs
  .readdirSync(path.resolve(__dirname, 'packages'), { withFileTypes: true })
  .filter((dirent) => dirent.isDirectory())
  .map((dirent) => dirent.name.replace(/s$/, ''))
const scopesCli = fs
  .readdirSync(path.resolve(__dirname, 'cli'), { withFileTypes: true })
  .filter((dirent) => dirent.isDirectory())
  .map((dirent) => dirent.name.replace(/s$/, ''))
const scopesDocs = fs
  ...
  .filter((dirent) => dirent.isDirectory())
  .map((dirent) => dirent.name.replace(/s$/, ''))
const scopesExample = fs
  .readdirSync(path.resolve(__dirname, 'example'), { withFileTypes: true })
  .filter((dirent) => dirent.isDirectory())
  .map((dirent) => dirent.name.replace(/s$/, ''))
...
module.exports = {
  ...
  prompt: {
    ...
    useEmoji: true,
    // scopes: [...scopesPackages,...scopesCli,...scopesDocs,...scopesExample],
    scopes: ['packages', 'cli', 'docs', 'example'],
