...
const createSrcIndex = (componentInfo: ComponentInfo) => {
  ...
  if (componentInfo.type === 'vue') {
    content = sfcTemplate(
      componentInfo.lineNameWithPrefix,
...
const createIndex = (componentInfo: ComponentInfo) => {
  fs.writeFileSync(
    `${componentInfo.fullPath}/index.ts`,
    indexTemplate(componentInfo)
  )
}
  ...
  new Promise((resolve, reject) => {
    ...
    execCmd(`cd ${componentInfo.fullPath} && pnpm init`)
      .then((r) => {
        ...
        // 4. 安装 utils 依赖
        execCmd(
          `cd ${componentInfo.fullPath} && pnpm install @${Config.COMPONENT_LIB_NAME}/utils`
        )
